...
import pl.estrix.frontend.web.user.MySessionInfo;
...
public abstract class MainController {
    ...
    @Autowired
    private ReadUserCommandExecutor readUserCommandExecutor;
    @Autowired
    ...
    private static ConcurrentHashMap<String, UserDto> userSessions = new ConcurrentHashMap<>();
    ...
    private static Map<String,String> basePaths = new HashMap<>();
...
//            UserDto u = ((MySessionInfo)sessionMap.get("mySessionInfo")).getCurrentUser();
//            users.put(session.getId(), u);
//        }
        if (!users.containsKey(session.getId())) {
            Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
            UserDto u = readUserCommandExecutor.findByLogin(authentication.getName());
            users.put(session.getId(), u);
        }

        if (basePaths.containsKey(session)){
            basePath  = basePaths.get(session);
        }else {
    ...
    public static void ClearUserSessions(String username){
//        userSessions.clear();
    }
